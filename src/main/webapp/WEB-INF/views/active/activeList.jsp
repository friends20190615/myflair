...
<div>
	<div class="brumbtext gray_666">
		<h1>  分享数据配置> <span class="bold">数据列表</span></h1>
	</div>
	<div class="p10">
	            <input name="input" type="button" class="blue_btn_bg" value="新增" onclick="openAddUserWindow()"/>
	</div>
	<div class="p0_10" >
	<form action="activelist.do" id="userform" name="userform"  onsubmit="return query();" method="post" style="display:none">
	<table class="tab_inquiry m10_0" cellpadding="0" cellspacing="0">
		<tr>
			...
			<th>编码：</th>
			<td><input type="text" name="code" id="code" class="ipt_150" value="${active.code}"/></td>
		</tr>
		<tr>
			<th>状&nbsp;&nbsp;&nbsp;&nbsp;态：</th>
			<td>
	...
	<input type="hidden" name="totalPage" id="totalPage" value="${totalPage}">
		...
		<table class="tab_list tr_even ml" cellpadding="0" cellspacing="0" width="100%">
			...
			<c:if test="${fun:length(list) > 0}">
				<c:forEach items="${list}" var="active">
					<tr>
					<td ><input type="radio" name="bmvid" id="bmvid${active.id}" value="${active.id}" class="checked_radio"/></td>
					<td>${active.code }</td>
					<td>${active.title }</td>
					<td>${active.hyContent }</td>
					<td>${active.pyqContent }</td>
					<td><fmt:formatDate value="${active.createTime }" pattern="yyyy-MM-dd HH:mm:ss" /></td>
					<td><fmt:formatDate value="${active.modifyTime }" pattern="yyyy-MM-dd HH:mm:ss" /></td>
					<td><a href="#" itemid="${active.id}" class="edit">修改</a> <a href="#" itemid="${active.id}" id="delete" class="delete">删除</a></td>
				</tr>
				</c:forEach>
			</c:if>
			</tbody>
...
$(document).ready(function(){
	...
	$(".delete").on("click",function(){
		if(confirm("您确定要删除该条记录？")){
			...
			$.ajax({
			    type: "POST",
			    url: "${baseContextPath}activeConfig/delete.do",
			    data: {id:id},
			    dataType: "json",
